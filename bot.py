...
def callback_handler(call):
    ...
    if data == "upload":
        ...
        sid = data.split(":")[1]
        s = get_script(sid)
        if s and s.get('original_file') and os.path.exists(s['original_file']):
        ...
        bot.edit_message_text(f"✅ Выдано user{target}", uid, call.message.message_id)
